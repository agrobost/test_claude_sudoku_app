    ...
    <SafeAreaView style={styles.safeArea} edges={['top', 'bottom']}>
      ...
      <ScrollView contentContainerStyle={styles.content}>
        <Section title={t('debug.state.title')}>
          ...
          <StateLine
            text={t('debug.state.analyticsActive', { value: onOff(analyticsActive()) })}
          />
        </Section>

        <Section title={t('debug.game.title')}>
          <StateLine
            text={t('debug.game.status', { status: gameStatus ?? t('debug.game.none') })}
          />
          <Action
        ...
        <Section title={t('debug.ads.title')}>
          ...
          <Action
            label={t('debug.ads.makeEligible')}
            onPress={run(debugMakeInterstitialEligible)}
          />
        ...
        <Section title={t('debug.data.title')}>
          <Action
            label={t('debug.data.winToday')}
            onPress={run(() => debugSeedDailyStreak(1))}
          />
          <Action
            label={t('debug.data.seedStreak')}
            onPress={run(() => debugSeedDailyStreak(7))}
          />
          <Action
          ...
          />
          <Action
            label={t('debug.data.refill')}
            onPress={() => void refillPuzzles().then(refresh)}
          />
